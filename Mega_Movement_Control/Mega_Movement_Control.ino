...
// SENSOR TIMING
// ==================================================
const unsigned long SENSOR_REPORT_INTERVAL = 150;  // fast stream to Raspberry
const unsigned long MPU_READ_INTERVAL      = 50;   // fast MPU for auto decisions
const unsigned long US_READ_INTERVAL       = 700;  // ultrasonic slower, display only
const unsigned long ALERT_COOLDOWN_MS      = 2000;
...
void setup() {
  ...
  Serial.println("MEGA:SENSOR_FAST_STREAM_ENABLED");
...
String detectAlert() {
  if (fabs(pitch) > PITCH_DANGER_DEG) return "TILT_DANGER";
  if (fabs(roll)  > ROLL_DANGER_DEG)  return "TILT_DANGER";

  if (frontUltrasonicCM > 0 && frontUltrasonicCM < OBSTACLE_FRONT_CM) return "OBSTACLE_FRONT";
  if (rearUltrasonicCM  > 0 && rearUltrasonicCM  < OBSTACLE_REAR_CM)  return "OBSTACLE_REAR";
